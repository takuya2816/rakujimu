...
def get_opening_hour(display_days, time_iter):  # {"2024/01/30": ["11:00","12:00"],...}
    opening_hour_dict = {}
    ...
    for display_day in display_days:
        display_day_dayofweek:int = datetime.datetime.strptime(display_day, '%Y/%m/%d').strftime('%A')
        ...
        for dayofweek_row in dayofweek_list:
            if dayofweek_list["dayofweek"] == match_dict[display_day_dayofweek]:
                times_list = []  # TODO:15分毎に営業時間を取得、ここからAPI叩いて確認
        opening_hour_dict[display_day] = times_list
    return opening_hour_dict


def get_reserved_list(display_days, time_iter):  # {"2024/01/30": ["11:00","12:00"],...}
    reserved_dict = {}
...
def lambda_handler(event, context):
    try:
        ...
        # 表示させる週初日の情報を取得
        display_days = event['display_days']  # ["2024/01/30", "2024/01/31", "2024/02/01"...]
        ...
        for (open_day, reserved_day), (open_times, reserved_times) in zip(opening_hour_dict, reserved_dict):
            reservable_dict[open_day] = open_times - reserved_times  # {"2024/01/30": ["11:00","12:00"],...}
